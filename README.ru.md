...
деревьев, там не остаётся, и ничто из того, что AGOT держит до 16-го, не тронуто.

| файл | что |
| --- | --- |
| `gfx/map/map_object_data/game_object_layers.txt` | семь слоёв AGOT; `activities_layer` 9 → 7, `AGOT_animal_layer` 9 → 8, `unit_layer` 9 → 10; четыре слоя зданий и дорог остаются на 16 |
...
| что выгружается / переключается | трава | активности, пена у берега | животные, горные и прочие эффекты окружения | `tree_low`, `tree_medium` | армии, начало заливки цветом, иконки крепостей и набегов, затухание тумана войны AGOT | крупные названия | владения, дороги, особые здания, `tree_high` |
