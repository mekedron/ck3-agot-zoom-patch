...
| файл | что |
| --- | --- |
| `gfx/map/map_object_data/game_object_layers.txt` | семь слоёв AGOT; `activities_layer` 9 → 7, `AGOT_animal_layer` 9 → 8, `unit_layer` 9 → 10, `building_layer` 16 → 10; особые здания и два слоя дорог остаются на 16 |
...
| что выгружается / переключается | трава | активности, пена у берега | животные, горные и прочие эффекты окружения | `tree_low`, `tree_medium` | владения, армии, начало заливки цветом, иконки крепостей и набегов, затухание тумана войны AGOT | крупные названия | дороги, особые здания, `tree_high` |
...
что модели уходят вместе с приходом цвета. 64 особых здания и объекты дорог немногочисленны
и остаются на 16-м, как у AGOT; `fade_out` — одно число на слой в `game_object_layers.txt`,
если хочется вернуть их подольше.
